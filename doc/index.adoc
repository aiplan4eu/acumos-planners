...
Now we make a Docker out of this setup. Here is the Dockerfile I use. We have to install coinor-libcbc-dev for optic-clp to run properly. We copy the various planner from the lcoal bin directory, as well as the planner_ACUMOS_bridge. We expose port 8061 to allow ACUMOS to talk to our component.
...
and you should get the result computed by the dockerized `planner_ACUMOS_bridge` and `ff`.
